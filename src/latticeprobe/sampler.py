...
def _sample_noise(params: LWEParams, rng: np.random.Generator) -> np.ndarray:
    """Sample one noise polynomial according to params."""
    if params.noise == "cbd":
        return cbd(params.eta, params.n, rng)
    if params.noise == "gaussian":
        return discrete_gaussian(params.sigma, params.n, rng)
...
def generate_batch(
    ...
    probability.  A fixed secret `secret` is used for all LWE samples so that
    the secret is shared within a split (caller must use a different secret for
    train vs test to enforce disjoint key sets — paper §5.2).
    ...
    # Generate a single secret for all LWE samples in this batch.
    if secret is None:
    ...
    for i in range(n_samples):
        if labels[i] == 1:
            a, b, _ = generate_lwe_sample(params, secret=secret)
        else:
